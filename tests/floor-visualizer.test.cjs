...
test('catalog matches all 25 Shipp-Mates names and existing swatch assets', () => {
  const catalog = JSON.parse(html.match(/const FLAKE_COLORS = (\[.*?\]);/s)[1]);
  const colors = fs.readFileSync(path.join(__dirname, '..', 'colors.html'), 'utf8');
  const names = new Set([...colors.matchAll(/data-name="([^"]+)"/g)].map(m=>m[1]));
  assert.equal(catalog.length, 25);
  assert.deepEqual(new Set(catalog.map(c=>c.name)), names);
  for(const color of catalog) for(const key of ['thumb','texture']) assert.ok(fs.existsSync(path.join(__dirname,'..',color[key])));
  assert.ok(!html.includes('showmeepoxy.com'));
  assert.ok(html.includes('/instant-quote.html?color='));
});

test('failed visualizer quote delivery retains the form and restores retry', async () => {
  const els = new Map(); const alerts=[];
  const c=vm.createContext({document:{getElementById(id){if(!els.has(id))els.set(id,{value:'Test value',style:{},disabled:false});return els.get(id)}},FormData,fetch:async()=>({ok:false}),alert:m=>alerts.push(m),fvState:{color:{name:'Lake Ozark'}},window:{location:{href:'https://shippmatesepoxy.com/floor-visualizer.html'}}});
  vm.runInContext(html.slice(html.indexOf('async function submitQuoteForm(){'),html.indexOf('// ---------- mobile menu')),c);
  await c.submitQuoteForm();
